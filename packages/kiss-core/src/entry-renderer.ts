...
export function renderEntry(desc: EntryDescriptor): string {
  ...
  b.push(`function stripLitComments(html) { return html.replace(/<!--\\/?(?:lit-part|lit-node)[^>]*-->/g, '') }`)
  ...
  b.push(`    '    try { const m = await loader(); if (m.default && !customElements.get(tag)) customElements.define(tag, m.default); }\\n' +`)
  b.push(`    '    catch(e) { console.warn("[KISS] Island <"+tag+"> hydration failed:", e); }\\n' +`)
  b.push(`    '  }\\n' +`)
  b.push(`    '  if ("requestIdleCallback" in window) requestIdleCallback(() => { for (const [t,l] of Object.entries(loaders)) hydrate(t,l); });\\n' +`)
  b.push(`    '  else setTimeout(() => { for (const [t,l] of Object.entries(loaders)) hydrate(t,l); }, 200);\\n' +`)
  b.push(`    '})();\\n' +`)
  b.push(`    '</script>'`)
  b.push(`}`)
