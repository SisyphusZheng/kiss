...
import { compileElementSpike } from '../src/internal/compiler/semantic-core/compile.ts';
...
const FIXTURE = new URL('../__fixtures__/compiled-element-spike/counter.tsx', import.meta.url);
...
const faults: Fault[] = [
  ...
  { label: 'when threshold', path: ['parts', 3, 'gt'], value: Number.POSITIVE_INFINITY },
...
Deno.test('Part Program validators independently fail closed across the artifact surface', async () => {
  ...
  const valid = compileElementSpike(source, '/project/app/islands/counter.tsx').program;
...
Deno.test('Part Program sink validators reject corrupted class, style, bool, html, ref and attr records', () => {
  const source = `
    ...
    export class SinkMatrix extends OpenElement {
      ...
      @property({ reflect: false, attribute: false }) bodyHtml = '<b>safe</b>';
      ...
      render() {
        return <main>
          ...
          <div innerHTML={this.bodyHtml}></div>
          <input ref={this.inputRef} />
        </main>;
      }
    }
  `;
  const valid = compileElementSpike(source, '/project/app/components/sink-matrix.tsx').program;
  validateCompilerProgram(valid);
  validateRuntimeProgram(valid);
...
Deno.test('Part Program event-action and item-slot grammars fail closed independently', () => {
  ...
  `;
  const valid = compileElementSpike(source, '/project/app/components/action-matrix.tsx').program;
  validateCompilerProgram(valid);
  validateRuntimeProgram(valid);
