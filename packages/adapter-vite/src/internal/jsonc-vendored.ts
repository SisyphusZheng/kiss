/**
 * Vendored @std/jsonc parse (jsr:@std/jsonc@1.0.2, MIT, Deno authors).
 *
 * Why vendored (#886): jsr: dependencies in adapter-vite break npm
 * consumers — deno pack emits an @jsr/std__jsonc dependency that npm
 * cannot resolve without a project .npmrc (@jsr:registry=https://npm.jsr.io),
 * and the emitted source keeps the '@std/jsonc' bare import, which neither
 * Node nor Deno resolves. Vendoring keeps the published package free of
 * jsr: deps with identical parse semantics.
 *
 * Ceiling: frozen at 1.0.2. Upgrade path: delete this file and re-add
 * "@std/jsonc": "jsr:@std/jsonc@^1.0.2" to deno.json imports once the
 * jsr→npm story for consumers is solved (e.g. deno pack rewriting imports
 * to @jsr/* specifiers). ponytail:
 */

// Copyright 2018-2025 the Deno authors. MIT license.
// This module is browser compatible.

/** The type of the result of parsing JSON (from @std/json). */
