...
// Read each starter template directly from the templates/ directory.
function readTemplate(key: string): string {
  return readFileSync(join(__dirname, '..', 'templates', key), 'utf-8');
}
...
Deno.test('create-open: gitignore hides generated build artifacts', () => {
  const gitignore = readTemplate('.gitignore');
...
Deno.test('create-open: starter includes default blog content directory', () => {
  const welcomePost = readTemplate('content/blog/welcome.md');
...
Deno.test('create-open: route index uses JSX-first definePage API', () => {
  const routeIndex = readTemplate('app/routes/index.tsx');
  ...
  assert(routeIndex.includes("from '@openelement/element'"));
...
Deno.test('create-open: freshness route records ISR cache intent', () => {
  const freshnessRoute = readTemplate('app/routes/freshness.tsx');
...
Deno.test('create-open: island counter uses JSX-first defineIsland API', () => {
  const islandCounter = readTemplate('app/islands/my-counter.tsx');
  ...
  assert(islandCounter.includes("from '@openelement/element'"));
...
Deno.test('create-open: generated project builds through the one-command pipeline', async () => {
  ...
  try {
    ...
    const aliases = [
      ...
      {
        find: '@openelement/protocol/',
        replacement: `${vitePath(join(repoRoot, 'packages', 'protocol', 'src'))}/`,
      },
      {
