...
源码包行为 `0.43.2`（`v0.43.2`）——ADR-0119 范围化接口冻结下的
在途五包源码行；已放弃的 beta 命名不再是当前版本线。
npm registry 行为 `v0.43.2`——已发布的五包版本(dist-tag `latest`)；registry 行允许比
源码行滞后一个 alpha。
...
五包收敛已作为 stable `0.43.2` 发布：它在 ADR-0135 Universal WC SSR 与
`0.43.1` 累计维护基线之上收敛运行时故障路径，并继续处于 ADR-0140 规定的
