...
export default class OpenPageRail extends OpenElement {
  ...
  override connectedCallback(): void {
    ...
    requestAnimationFrame(() => { this.#buildAutomaticOutline(); this.#activate(); });
  }
  override disconnectedCallback(): void { this.#observer?.disconnect(); super.disconnectedCallback(); }
  #activate(): void {
    this.#links = [...this.querySelectorAll<HTMLAnchorElement>('a[href^="#"]')];
    const targets = this.#links.map((link) => document.getElementById(link.hash.slice(1))).filter((target): target is HTMLElement => Boolean(target));
    if (!targets.length || !('IntersectionObserver' in window)) return;
    this.#observer = new IntersectionObserver((entries) => {
      const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => a.boundingClientRect.top - b.boundingClientRect.top)[0];
      if (!visible?.target.id) return;
      for (const link of this.#links) link.toggleAttribute('aria-current', link.hash === `#${visible.target.id}`);
  ...
  #buildAutomaticOutline(): void {
    ...
    for (const [index, heading] of headings.entries()) {
      if (!heading.id) heading.id = `section-${index + 1}-${heading.textContent?.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'section'}`;
  ...
  override render() { return <details open><summary>On this page</summary><nav class='links' aria-label='On this page'><slot><a href="#start">Overview</a></slot></nav></details>; }
}
