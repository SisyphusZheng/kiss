...
codex/v044-1156-ci-doc-governance
...
```text
$ grep -c -i -E 'kimi|gpt|k3-256k|moonshot|KimiCode|Codex' \
...
docs/prompts/v0.44.0-SOL-ORCHESTRATOR.md:12
docs/adr/ADR-0146-three-role-agent-execution-control-plane.md:8
.agents/v044-kimi-implementer.md:3
.agents/v044-kimi-release-verifier.md:2
exit=0

$ grep -n -i -E '\bsol\b|kimi|gpt|k3' docs/current/v0.44.0-EXECUTION-PLAN.md | head -12
29:plus its tests and documentation. A packet should fit one K3 session. Split it when:
42:| `alpha.0` | #1160 → #1157 → #1158 → #1159 → #1156 → #1182 control-plane closure | fresh K3      |
...
1. `docs/current/v0.44.0-EXECUTION-STATE.json` (permanently forbidden) contains
   `"command": "kimi"`, `"model": "kimi-code/k3-256k"`, and `.agents/v044-kimi-*.md`
   agentFile paths. Packet test-first item 1 requires the new check to fail on brand
...
3. `docs/prompts/v0.44.0-SOL-ORCHESTRATOR.md` and
...
4. `.agents/v044-kimi-implementer.md` and `.agents/v044-kimi-release-verifier.md` (both
...
- amend the permanently forbidden list (or execute the control-plane migration itself) for
  ...
  `.agents/v044-kimi-implementer.md` / `.agents/v044-kimi-release-verifier.md`; or
