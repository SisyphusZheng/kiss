...
The user explicitly authorized Sol to perform this one control-plane bootstrap because
...
- Both exact Agent files now contain valid KimiCode Markdown frontmatter.
...
- KimiCode 0.38.0 prompt mode successfully loads both profiles using
  `kimi-code/k3-256k` and returns distinct role markers.
...
`unblock-contract.md` records the repair contract derived from Kimi's installed CLI and
official documentation. This summary records the later, explicitly authorized bootstrap
resolution.

## Residual risk

The real profile smoke performs authenticated Kimi calls. That is intentional for the
