...
## Current Version Line: v0.40.1 Released (Elements + Preact + Repository Slimming)

v0.40.1 is released as the product-line cleanup release. It keeps the public
target at `openElement = Elements + UI + Framework + Protocols`, makes the
active package line `0.40.1`, and narrows the workspace to 11 packages. Hub,
...
v0.40.1 breaking cleanup train consolidated into the v0.40.1 release. AutoFlow3
...
Local v0.40.1 release-readiness evidence passes: `fmt:check`, `lint`, `typecheck`, `test`,
...
The active implementation line is now v0.40.1. It proceeds from v0.39.0
...
- `docs/release/v0.40.1-product-line-cleanup.md`
...
| v0.40.1 | Elements + Preact + Repository Slimming            | Release-ready       | Productize `OpenElement`, prove Preact islands, and collapse root/docs/Hub/package/gate shape to 11 packages  |
...
to local version **0.40.1**. Published package availability is completed by the
...
- **Signal engine default.** `@preact/signals-core` is the default engine
  (since v0.40.1). `alien-signals` remains available as an optional engine
