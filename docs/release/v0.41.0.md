# v0.41.0 npm Distribution via `deno pack` + WC Interop

**Deno is the development and release toolchain. Published packages remain pure ESM and runtime-agnostic.**

## Summary

v0.41.0 migrates openElement's package distribution channel from JSR to npm
using Deno 2.8+ `deno pack` as the build tool, hardens the signal-DOM
architecture, proves cross-framework Web Components interoperability, and
hardens the App/protocol architecture so OpenElement App owns route/render/request
concepts while Vite/Hono/Nitro remain official adapters. Artifacts are built with
...
- Keep `wait-jsr-release-metadata.ts` as a historical observation tool, not a
  release gate (`jsr-consumer-monitor.yml` was removed in #352).
...
- GitHub Actions `autoflow-release.yml` successfully publishes to npm with
  provenance; no required JSR publish step is part of release closure.
...
- `v0.41.0-alpha.6` — App/protocol architecture hardening + front-half cleanup audit closure
- `v0.41.0-alpha.7` — Mac Mastodon Desktop incubation
- `v0.41.0-beta` — Breaking interface-depth and adoption closure; beta.1–beta.3
  are withdrawn partial npm artifacts, first coherent candidate is beta.4
