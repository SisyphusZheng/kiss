# v0.42.0-alpha.3

## Revalidation continuity

The third alpha of the 0.42 line (ADR-0120): the enhanced submit no
longer reloads the world. `data-open-enhance` forms fetch the same HTML
the no-JavaScript path renders (303/422), morph it into place, and follow
the PRG target with `history.pushState`.

- **Island survival**: a hydrated island whose light-DOM surface is
  unchanged in the incoming document keeps its shadow state (the DSD
  template child is excluded from the comparison). Islands whose surface
  changed are replaced; `data-open-preserve` exempts any subtree; the
  island client script is never re-executed by a morph.
- **Named regions**: a `data-open-region` container limits the morph to
  the matching region when present.
- The `ActionResult` JSON path remains for custom clients; the built-in
  enhancement uses HTML morphing for continuity.

## Verification

- Survival matrix: a counter island at 3 survives both a 422 validation
  morph and a PRG success morph; static/request-time mixed navigation —
  Chromium, Firefox and WebKit, 36/36.
- Full release-tier gates green; npm `alpha`/`latest` at `0.42.0-alpha.3`;
  post-publish consumer and third-party WC smoke.

## Migrating from alpha.2

Nothing required. Forms already marked `data-open-enhance` gain morphing
automatically; if you relied on the JSON `ActionResult` responses, they
are unchanged for `x-openelement-action` callers.

---

AutoFlow3 patch release evidence: `publish-existing-v0.42.0-alpha.3-2026-07-27T08-43-31-362Z`.

- Previous package line: `0.42.0-alpha.2`
- Released package line: `0.42.0-alpha.3`
- Policy version: `autoflow3-v0`
- Status: `completed`

`/@fs/` Windows verification status: see `docs/current/HYDRATION_CONTRACT.md` (Known limitations).

## Errata (2026-07-27, alpha.5)

- **Test count**: the request-time fixture suite at tag `v0.42.0-alpha.3`
  has 14 tests × 3 engines = 42 runs, not 36 — the "36/36" figure above
  does not match the durable tag.
- **Island-survival claims were false at release time**: the two morph
  island-survival specs fail deterministically at this tag. The
  enhancement's document-level submit listener never fired for forms
  inside page-element shadow roots (`submit` is not a composed event), so
  the claimed morph survival was never exercised — the specs were silently
  passing through the native no-JS path. The suite also had no CI gate.
  Root cause fixed in `0.42.0-alpha.5` (ADR-0121); see the alpha.5
  release note.

## Evidence

- passed: verify published source version (exit 0)
- passed: verify main CI success for HEAD (exit 0)
- passed: package artifact gate (exit 0)
- passed: publish npm packages (exit 0)
- passed: verify npm versions and dist-tags (exit 0)
- passed: post-publish npm consumer smoke (exit 0)
- passed: post-publish third-party Web Component smoke (exit 0)
- passed: stage release evidence (exit 0)
- passed: commit release evidence (exit 0)
- passed: tag release (exit 0)
- passed: push tag (exit 0)
- passed: create GitHub release (exit 0)

## Durable closure

- Immutable tag commit: `a4daf7749227d3b07aeb305b21098a5347437512`
- Final completed evidence commit: `ef5f1454c294c5c957ebe1c519f296f4dfd46231`
- Successful release run: https://github.com/open-element/openelement/actions/runs/30250944865
- GitHub release: https://github.com/open-element/openelement/releases/tag/v0.42.0-alpha.3
