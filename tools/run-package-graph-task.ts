...
async function assertCleanWorktree(): Promise<void> {
  ...
  // Release evidence files are generated by the release flow itself; untracked
  // docs/release entries should not block publishing.
  const nonEvidenceDirty = status
    .split(/\r?\n/)
    .filter((line) => !/^\?\? docs\/release\//.test(line))
    .join('\n');
  if (nonEvidenceDirty) {
